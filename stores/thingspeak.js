...
ThingSpeakStore.prototype.configure_thing = function (thing, ad, callback) {
...

    {
        ...
        v = meta.get(ts_channel_write_key, null);
        if (v !== null) {
            promptd['default'] = v;
        ...
        v = meta.get(ts_channel_read_key, null);
        if (v !== null) {
            promptd['default'] = v;
...

    {
        var cd = {};
        for (var field = 1; field <= 8; field++) {
            code = meta.get(ts_channel_fields + "/" + field, null);
            if (code) {
                cd[code] = field;
            }
        }
